...
var DefaultSkipFiles = []string{
	"extracted" + string(os.PathSeparator) +"DerivedName.txt", // rune <-> it's name; its better to work with this file in separate style
	"extracted" + string(os.PathSeparator) +"DerivedNumericValues.txt", // numeric rune <-> its math value; its better to work with this file in separate style
	"LineBreak.txt" , // covered by "extracted/DerivedLineBreak.txt"
	"ArabicShaping.txt",        // covered by extracted JoiningGroup & JoiningType
	"BidiCharacterTest.txt",    // just test file
	"BidiTest.txt",             // just test file
	"NamedSequences.txt",       // not a runes data
	"NamedSequencesProv.txt",   // not a runes data / empty file
	"NormalizationTest.txt",    // just test file
	"NushuSources.txt",         // not a runes data
	"PropertyAliases.txt",      // not a runes data
	"PropertyValueAliases.txt", // not a runes data
	"auxiliary" + string(os.PathSeparator) +"GraphemeBreakProperty.txt", // just test file
}
