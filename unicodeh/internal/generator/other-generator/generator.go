...
// variable names
const (
	varBidiPairedBracket  = "bidiPairedBracket"
...
func generateBidiBracketsPublic(data *bytes.Buffer) {
	...
	data.WriteString("var " + strings.Title(varBidiPairedBracket) + "=" + varBidiPairedBracket + "\n\n")
}
...
func generateBidiBrackets(srcDir string, data *bytes.Buffer) {
	...
	p := ucd.New(src)
	// Parse src
	data.WriteString("var " + varBidiPairedBracket + " = map[rune]rune{")
	for p.Next() {
		bracket0 := p.Rune(0)
		bracket1 := p.Rune(1)
